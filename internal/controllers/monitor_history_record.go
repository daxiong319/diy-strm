...
// ---------------------------------------------------------------------------
// 监控历史埋点工具（TG 频道订阅 / RE0订阅 / TG 机器人三类入口共用）
// 与参考实现对齐：每条监控转存尝试（成功/失败/跳过）各写一条审计记录
// ---------------------------------------------------------------------------

// MonitorMediaMeta 监控历史记录的影片关联信息（可选，通过 variadic 传入，
// 不传则留空）。仅RE0订阅 / TG 影视订阅能关联影片，其他入口可不传。
...
// 每 5 分钟轮询都会重复入库刷屏（对齐 参考实现的 is_message_processed：每消息只审计一次）。
// RE0入口 messageURL 恒为空、机器人入口 subID=0，均不满足条件，行为不变。
