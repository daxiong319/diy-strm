...
此类场景请在该订阅中开启 **「回溯搜索」** 开关：执行时忽略频道游标，从频道历史帖里全文匹配并转存。引擎按 t.me/s `?before=` **向后翻页**回看历史（复刻参考实现 网页通道），页数上限可在「回溯页数」设置（默认 50 页，每页约 20 帖，最大 200；普通增量在两次运行间隔超过 20 帖时也会自动翻页补齐缺口，最多 100 页）。已转存过的影片 / 链接自动去重，重复执行安全，且回溯不会推进频道游标、不影响其它订阅的追新。

回溯匹配细节：
- 频道帖子里的按钮 / 实体链接（`<a href>`）与正文一起参与解析，网盘分享链接从按钮与解密后明文都能提出。
- 资源帖若把分享暗号加密成 `[ENCRYPTED_LINK_START]...[ENCRYPTED_LINK_END]`，引擎按 同款算法自动解密后再提取链接（AES-256-CBC，默认密钥 `123456` 右侧补 `0` 至 32 字节；如与原版不同源可在环境变量设置 `TG_CHANNEL_LINK_ENC_KEY` 覆盖）。
- 帖内出现 `123FSLinkV1/V2`、`123FLCPV1/V2` **秒传暗号**时（参考实现 需配合 123 秒传 API 转存），目前识别并留痕提示、**不自动转存**。
- 回溯未命中的帖子（含本网盘链接但关键词未命中 / 带秒传暗号暂不支持）会以「跳过」状态写入**监控历史**，可在监控历史页查看跳过原因定位断点。
- 频道失效或无法公开访问（t.me 重定向回首页）时会在执行摘要报错提示。

## 🎥 观影（Guanying）使用说明

**启用与登录**：发现 → 基础配置 → 观影 → 开启并登录（保存后会话加密存储，PoW 验证自动重验，无需手动维护）。

**影视探索目录**：发现 → 影视探索 → 选「观影」标签，可切换「最近更新的电影 / 最近更新的剧集」，支持分页翻页浏览（最多 5 页）；条目点击跳转观影站详情页。

**在线播放**：作品详情 → 关联资源 → 观影源下会出现「在线播放」线路卡片（西瓜线路、暴风线路等），点「在线播放」跳转观影站自带播放器（ArtPlayer + HLS）观看全剧集；同页也可同时查看网盘分享、磁力资源并做转存/离线。

**提示**：观影官方域名不定期更换且全部套 PoW 验证墙，客户端从防失联页自动解析可用域名并自动完成 RSW 验证；若检索报「安全验证已过期且自动恢复失败」，稍等自动重试或点「重新匹配」。
