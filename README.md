...
此类场景请在该订阅中开启 **「回溯搜索」** 开关：执行时忽略频道游标，从频道历史帖里全文匹配并转存。引擎按 t.me/s `?before=` **向后翻页**回看历史（复刻 tgto123 网页通道），页数上限可在「回溯页数」设置（默认 50 页，每页约 20 帖，最大 200；普通增量在两次运行间隔超过 20 帖时也会自动翻页补齐缺口，最多 100 页）。已转存过的影片 / 链接自动去重，重复执行安全，且回溯不会推进频道游标、不影响其它订阅的追新。

回溯匹配细节：
- 频道帖子里的按钮 / 实体链接（`<a href>`）与正文一起参与解析，网盘分享链接从按钮与解密后明文都能提出。
- 资源帖若把分享暗号加密成 `[ENCRYPTED_LINK_START]...[ENCRYPTED_LINK_END]`，引擎按 tgto123 同款算法自动解密后再提取链接（AES-256-CBC，默认密钥 `123456` 右侧补 `0` 至 32 字节；如与原版不同源可在环境变量设置 `TG_CHANNEL_LINK_ENC_KEY` 覆盖）。
- 帖内出现 `123FSLinkV1/V2`、`123FLCPV1/V2` **秒传暗号**时（tgto123 需配合 123 秒传 API 转存），目前识别并留痕提示、**不自动转存**。
