...
- 跨会话记忆：会话开始前及每个任务开始前，优先阅读 Obsidian 记忆库 `D:\6.2号工作资料\lifeOS\1. 项目\AI 协作记忆\` 中对应项目的记忆文件（含服务器部署、镜像名、密钥配置、排障经验等关键事实）以恢复上下文；禁止直接以"没有、找不到、不记得"推脱，若确实缺失应先说明查阅结论再询问。会话中产生新的关键事实、决策或排障经验时，必须追加写入对应记忆文件；用户明确告知的重点信息（镜像名、服务器、密钥、账号、偏好等）当场写入，不得依赖会话内记忆。
