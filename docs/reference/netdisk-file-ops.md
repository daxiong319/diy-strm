...
- **新文件更优** → 删除匹配的旧版本后放入新文件（洗版替换）。
- **持平或更差** → 保留库内版本，删除待整理目录中的新文件（同质量绝不堆积副本，云盘同名自动改名不会触发）。质量维度全部打平时用**文件大小**兜底决胜（大小与分辨率/码率正相关；`organizeEntry.Size` 由各网盘列表接口填充）。
- 洗版动作写应用日志（`洗版：新版本质量更优，删除旧版本 …` / `同集旧版本质量不低于新版本，删除新文件 …`）与整理纪录。

整理命名追加原始文件的质量标签段（对齐云盘自动整理格式）：剧集 `标题.年份.S01E02.第2集.1080p.WEB-DL.AAC2.0.H.264.mp4`，电影 `标题 (年份).1080p.WEB-DL.mkv`。标签用 `extractQualityTags(原始文件名, TMDB officialTitle)` 提取（不用 `ParseMedia` 的标题，中英混合名可能解析出垃圾标题残留）；无可解析标签保持原命名；文件名已带质量标签时不重复追加。
