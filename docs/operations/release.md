...
推送 `dev` 还会触发 `beta.yaml`，发布多架构镜像 `ghcr.io/<owner>/diy-strm:beta` 和 `afengj/diy-strm:beta`。推送 `feature/**` 还会触发 `feature.yaml`，发布 `ghcr.io/<owner>/diy-strm:<branch-tag>` 和 `afengj/diy-strm:<branch-tag>`：分支名会去掉 `feature/` 前缀、转为小写，斜杠和非法字符替换为连字符，最长 120 个字符。`dev` 的同一分支构建会取消仍在运行的旧 beta 构建。
...
发布流程还会使用 `GITHUB_TOKEN` 推送 GHCR 镜像 `ghcr.io/<owner>/diy-strm:<tag>` 和 `ghcr.io/<owner>/diy-strm:latest`，并使用 Secrets `DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN` 推送 DockerHub 镜像 `afengj/diy-strm:<tag>` 和 `afengj/diy-strm:latest`。
